...
public class IntroloopTrack : MonoBehaviour
{
	...
	internal bool Resume()
	{
		...
		if(!music.nonLooping && !music.loopWholeAudio) //If contain intro
        {
            if(isPausingOnIntro)
            {
                remainingTime = music.IntroLength - twoSources[sourceToContinuePlaying].time;
            }
            else
            {
                remainingTime = music.IntroLength + music.LoopingLength - twoSources[sourceToContinuePlaying].time;
            }
        }
        else
        {
            remainingTime = music.ClipLength - twoSources[sourceToContinuePlaying].time;
        }
	...
	private void ScheduleNextLoop()
	{
		...
		twoSources[nextSourceToPlay].time = music.introBoundary;
